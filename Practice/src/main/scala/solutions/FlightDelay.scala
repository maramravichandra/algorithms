...
object FlightDelay {
  ...
  def main(args: Array[String]) {
    ...
    def parseFlight(str: String): Flight = {
      val line = str.split(",")
      Flight(line(0), line(1), line(2), line(3), line(4).toInt, line(5), line(6), line(7),
    ...
    val textRDD = sc.textFile("C:\\Users\\KOGENTIX\\New folder\\Xebia\\src\\main\\resources\\flightdata.csv")
    ...
    val fltCountsql = sqlContext.sql("select dofM, dest, count(flnum) as total  FROM flights GROUP BY dofM, dest  ORDER BY total DESC")
    //find average delay
    val avgdepdel = sqlContext.sql("SELECT origin, avg(depdelaymins) as avgdepdelay FROM flights GROUP BY origin ORDER BY avgdepdelay DESC")
    ...
    println("=============================================================================")
    println("======================    SOME OTHER TESTING    =============================")
    println("=============================================================================")
    
    
    val groupedRDD = flightsRDD.mapPartitions{ itr =>
      itr
    }
