...
object AadhaarNumberGeneration {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val aadharSchama = new StructType().add("date", DataTypes.DateType, false)
      .add("registrar", DataTypes.StringType, false)
      .add("private_agency", DataTypes.StringType, false)
      .add("state", DataTypes.StringType, false)
      .add("district", DataTypes.StringType, false)
      .add("sub_district", DataTypes.StringType, false)
      .add("pincode", DataTypes.StringType, false)
      .add("gender", DataTypes.StringType, false)
      .add("age", DataTypes.IntegerType, false)
      .add("aadhaar_generated", DataTypes.IntegerType, false)
      .add("rejected", DataTypes.IntegerType, false)
      .add("mobile_number", DataTypes.IntegerType, false)
      .add("email_id", DataTypes.IntegerType, false);
    ...
    val mismatchedDF = states.except(states);
    mismatchedDF.createOrReplaceTempView("mismatchedTable");
    ...
    def setStatus( column1:Any, column2:Any )={
      ...
      if( column1.toString().equalsIgnoreCase(column2.toString())){
        status = column1.toString()+" - "+column2.toString()+" : EQUAL"
      }else{
    ...
    sparkSession.udf.register("getStatus", setStatus _)
    
    val columns = mismatchedDF.schema.fields.map( fld => s"""getStatus(a.${fld.name},b.${fld.name}) as ${fld.name}_status""").mkString(",");
    
    val finalDF = sparkSession.sql(s"""SELECT $columns FROM table1 a,table2 b where a.primarykey=b.primarykey """);
    finalDF.show();
