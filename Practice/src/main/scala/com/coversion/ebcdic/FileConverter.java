...
public class FileConverter {
  ...
  private Charset ebcdicCharset = CP1047;
  private Charset outputCharset = Charset.forName("ASCII");
  ...
  public void setEbcdicCharset(String ebcdicCharset ){
	  this.ebcdicCharset = Charset.forName(ebcdicCharset); 
  }
  ...
  void convert(File ebcdicInputFile, File convertedOutputFile) throws Exception {
    ...
    try{
      ...
      System.out.println(" ebcdicCharset "+ebcdicCharset);
      convert(ebcdicInput, writer);
      
      
      
    }catch (Exception e){
      throw new Exception("Unable to convert file " + ebcdicInputFile.getAbsolutePath(), e);
    }finally{
      close(writer);
    }
  }
  ...
  public void close(Closeable closeable) throws Exception {
    try {
      ...
      throw new Exception("Unable to close", e);
    }
  }
  ...
  private void convert(int[] ebcdicInput, Writer convertedOutputWriter) throws IOException {
    ...
    for (int index = 0; index < ebcdicInput.length; index++) {
      ...
      if (fixedLength != -1 && index > 0 && index % fixedLength == 0) {
        ...
        appendToFIle((char) LF, convertedOutputWriter );
      }
      ...
      if( numEmpty <= 2 ){
    	  appendToFIle((char) convertedChar, convertedOutputWriter );
      }
    }
  }
  
  private void appendToFIle(char letter,Writer convertedOutputWriter) throws IOException{
	  convertedOutputWriter.append(letter);
  }
  ...
  private int replaceNonPrintableCharacterByWhitespace(int character) {
    ...
    for (char nonPrintableChar : NON_PRINTABLE_EBCDIC_CHARS) {
      if (nonPrintableChar == (char) character ) {
    	  System.out.println("nonPrintableChar "+nonPrintableChar +", character"+(char) character);
        return WS;
      }
    }
    return character;
