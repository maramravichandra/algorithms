...
//    val primKey = splitStr(0).replace("SELECT", "").replace("(", "").replace(")", "").replace("~", "__")
//    val query = "SELECT " + splitStr(1) 
//    println("primary key", primKey );
//    println(" Query",query);
//    
//    return 
//    val a = 5;
...
//    return;
    ...
    var df1 = spark.read.format("com.databricks.spark.csv").schema(schema).load("C:\\Users\\KOGENTIX\\New folder\\Xebia\\src\\main\\resources\\compare1.csv").orderBy("id");
    var df2 = spark.read.format("com.databricks.spark.csv").schema(schema).load("C:\\Users\\KOGENTIX\\New folder\\Xebia\\src\\main\\resources\\compare2.csv").orderBy("id");
    df1.show();
